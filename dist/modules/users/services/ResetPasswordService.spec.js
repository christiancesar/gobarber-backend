...
describe('ResetPasswordService', () => {
  ...
  it('should be able to reset the password', async () => {
    ...
    expect(generateHash).toHaveBeenCalledWith('123123'); // expect(updateUser?.password).toBe('123123');
  });
  ...
  it('should not be able to reset password if passed more than 2 hours', async () => {
    ...
    const newDate = jest.spyOn(Date, 'now').mockImplementationOnce(() => {
    ...
    await expect(resetPassword.execute({
      password: '123123',
      token
    })).rejects.toBeInstanceOf(_AppError.default); // expect(updateUser?.password).toBe('123123');
  });
});